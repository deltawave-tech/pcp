...
- [ ] Make `tokenizer="char"` deterministic and checkpointed
  ...
  - **Impact:** Resuming from a checkpoint can silently scramble token IDs (embedding rows / output logits no longer correspond to the same characters), and “PCP vs PyTorch” dataset-based parity becomes unreliable. If the dataset has >65 unique chars, which chars get dropped can also vary (`MAX_VOCAB_SIZE=65`), changing the effective dataset.
