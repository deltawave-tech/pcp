...
pub fn build(b: *std.Build) void {
    ...
    const test_step = b.step("test", "Run native fast unit tests");
    const test_native_step = b.step("test-native", "Run native fast unit tests");
    ...
    const test_cross_step = b.step("test-cross", "Compile and run unit tests for supported cross targets when available");
    for (cross_test_targets) |t_target| {
        const cross_unit_tests = b.addTest(.{
            .root_source_file = b.path("src/unit_tests.zig"),
            .target = b.resolveTargetQuery(t_target),
            .optimize = optimize,
        });
        const run_cross_unit_tests = b.addRunArtifact(cross_unit_tests);
        run_cross_unit_tests.skip_foreign_checks = true;
        test_cross_step.dependOn(&run_cross_unit_tests.step);
    }
    ...
    // Qwen3.6 27B one-token smoke runner.
    const qwen36_27b_smoke = b.addExecutable(.{
        .name = "qwen36_27b_smoke",
        .root_source_file = b.path("src/examples/qwen36_27b_smoke.zig"),
        .target = target,
        .optimize = optimize,
    });
    qwen36_27b_smoke.root_module.addImport("pcp", pcp_module);
    addIreeDependencies(qwen36_27b_smoke, b, iree_config);
    b.installArtifact(qwen36_27b_smoke);

    const run_qwen36_27b_smoke_cmd = b.addRunArtifact(qwen36_27b_smoke);
    if (b.args) |args| run_qwen36_27b_smoke_cmd.addArgs(args);
    run_qwen36_27b_smoke_cmd.step.dependOn(&qwen36_27b_smoke.step);

    const run_qwen36_27b_smoke_step = b.step("run-qwen36-27b-smoke", "Run Qwen3.6-27B one-token VMFB smoke");
    run_qwen36_27b_smoke_step.dependOn(&run_qwen36_27b_smoke_cmd.step);
    ...
    if (b.args) |args| {
        run_isolated_vjp_tests_cmd.addArgs(args);
    }
    ...
    // GRPO backward builder - development gate for exported forward MLIR.
    const grpo_backward_builder = b.addExecutable(.{
        .name = "grpo_backward_builder",
        .root_source_file = b.path("src/examples/grpo_backward_builder.zig"),
        .target = target,
        .optimize = optimize,
    });
    grpo_backward_builder.root_module.addImport("pcp", pcp_module);
    addIreeDependencies(grpo_backward_builder, b, iree_config);

    const run_grpo_backward_builder_cmd = b.addRunArtifact(grpo_backward_builder);
    if (b.args) |args| {
        run_grpo_backward_builder_cmd.addArgs(args);
    }
    run_grpo_backward_builder_cmd.step.dependOn(&grpo_backward_builder.step);

    const run_grpo_backward_builder_step = b.step("build-grpo-backward", "Build GRPO backward MLIR from an exported forward MLIR");
    run_grpo_backward_builder_step.dependOn(&run_grpo_backward_builder_cmd.step);
}
