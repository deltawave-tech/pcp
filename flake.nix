  ...
  outputs = { self, nixpkgs, zig-overlay, zls, flake-utils }@inputs:
    flake-utils.lib.eachSystem [
      ...
      in rec {
        ...
        packages.pcp-python-runtime = pkgs.python3.withPackages (ps: [
          ps.transformers
          ps.wandb
        ]);
        ...
        packages.pcp = llvmPkg.stdenv.mkDerivation {
          ...
          postFixup = ''
            ...
            SYSTEM_RPATH="/usr/lib/x86_64-linux-gnu/nvidia/current:/usr/lib/x86_64-linux-gnu:/usr/lib64:/usr/lib:/opt/amdgpu/lib/x86_64-linux-gnu:/opt/amdgpu/lib:/run/opengl-driver/lib:/opt/rocm/lib:/opt/rocm/hip/lib"

            for bin in $out/bin/*; do
              if [ -f "$bin" ] && patchelf --print-interpreter "$bin" >/dev/null 2>&1; then
                patchelf --force-rpath --set-rpath "$NIX_RPATH:$SYSTEM_RPATH" "$bin"
              fi
            done

            wrapProgram $out/bin/pcp \
              --prefix PATH : "${packages.pcp-python-runtime}/bin" \
        ...
        packages.pcp-runtime-files = pkgs.runCommand "pcp-runtime-files" { } ''
          mkdir -p "$out/app/tools" "$out/models" "$out/etc/pcp"
          cp ${./tools/qwen_tokenizer_server.py} "$out/app/tools/qwen_tokenizer_server.py"
          cp ${./tools/wandb_adapter.py} "$out/app/tools/wandb_adapter.py"
          chmod +x "$out/app/tools/qwen_tokenizer_server.py" "$out/app/tools/wandb_adapter.py"
        '';
        ...
        packages.pcp-container-root = pkgs.buildEnv {
          name = "pcp-container-root";
          paths = [
            packages.pcp
            packages.pcp-python-runtime
            packages.pcp-wandb-adapter
            packages.pcp-runtime-files
            packages.pcp-container-entrypoint
          ];
          pathsToLink = [ "/bin" "/app" "/models" "/etc" ];
        };

        packages.pcp-docker = pkgs.dockerTools.buildImage {
          name = "pcp";
          copyToRoot = packages.pcp-container-root;
          config = {
            Entrypoint = [ "/bin/pcp-entrypoint" ];
            Cmd = [ "--help" ];
            WorkingDir = "/app";
            Env = [
              "PATH=/bin"
            ...
            Labels = {
              ...
              "org.opencontainers.image.description" = "PCP runtime with Python tokenizer and telemetry helpers";
            };
          };
        };
