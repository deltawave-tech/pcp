...
const wandb = @import("../observability/wandb.zig");
...
const RECOVERY_FILENAME = "worker_fabric_state.bin";
...
pub const DiLoCo = struct {
    allocator: Allocator,
    coordinator: *WorkerFabricController,
    config: DiLoCoConfig,
    ...
    worker_scope: ?[]message.NodeId,

    const Self = @This();

    // Change the init signature to accept the generic executor (dataset now loaded by workers)
    pub fn init(allocator: Allocator, coordinator: *WorkerFabricController, config: DiLoCoConfig, executor: Executor, mlir_builder: *MLIRBuilder) !Self {
    ...
    pub fn setWorkerScope(self: *Self, worker_ids: []const message.NodeId) !void {
        self.clearWorkerScope();
        self.worker_scope = try self.allocator.dupe(message.NodeId, worker_ids);
    }

    pub fn clearWorkerScope(self: *Self) void {
        if (self.worker_scope) |scope| {
            self.allocator.free(scope);
            self.worker_scope = null;
        }
    }

    fn ensureScopedWorkersCompiled(self: *Self) !void {
        if (self.worker_scope) |scope| {
            try self.coordinator.ensureWorkersCompiledForIds(
                scope,
    ...
    fn snapshotScopedWorkers(self: *Self) !ArrayList(training_controller.WorkerConnection) {
        if (self.worker_scope) |scope| {
            return self.coordinator.snapshotWorkersByIds(scope);
        }
        return self.coordinator.snapshotWorkers();
    }
    ...
    pub fn run(ptr: *anyopaque) !void {
        ...
        // --- NEW LOGIC: Delegate compilation/distribution to WorkerFabricController ---
        // Ensure initial workers are ready before starting
        try self.ensureScopedWorkersCompiled();
        // --- END NEW LOGIC ---
        ...
        for (0..self.config.base_config.outer_loop_steps) |step| {
            ...
            const participants = try self.snapshotScopedWorkers();
            ...
            monitoring.setMetrics(self.current_epoch, self.metrics.loss, round_dispatch.workers_assigned);
            if (self.control_state) |state| {
    ...
    fn broadcastToSnapshot(self: *Self, workers: []const training_controller.WorkerConnection, round_id: message.RoundId) !RoundDispatch {
        ...
        if (self.config.dtype == .f32) {
            var offset: usize = 0;
            while (offset < raw_bytes.len) : (offset += @sizeOf(f32)) {
                const bits = std.mem.readInt(u32, raw_bytes[offset..][0..@sizeOf(f32)], .little);
        ...
        std.log.warn("WORKER_FABRIC SEND CHECK:", .{});
        ...
        var worker_ids = ArrayList(message.NodeId).init(self.allocator);
        defer worker_ids.deinit();
        for (workers) |worker| {
            try worker_ids.append(worker.node_id);
        }
        ...
        if (use_chunked_transfer) {
            ...
            try self.coordinator.broadcastLargeDataToWorkerIdsWithContext(worker_ids.items, raw_bytes, .{
        ...
        for (workers) |worker| {
            ...
            var assignment_value = try data_assignment.toJsonValue(self.allocator, .{
            ...
            defer assignment_value.deinit();
            try payload_map.put(data_assignment.field_name, assignment_value.value);
    ...
    fn accumulateAndApplyGradients(self: *Self, dispatch: RoundDispatch) !void {
        ...
        const accumulator = try self.allocator.alloc(f32, num_floats);
        ...
        while (collected_count < dispatch.workers_assigned) {
            ...
            var owned_chunked_update: ?training_controller.ReassembledUpdate = null;
            defer if (owned_chunked_update) |*update| update.deinit();

            if (is_chunked) {
                owned_chunked_update = (try self.coordinator.takeReassembledUpdateForMessage(response)) orelse {
                ...
                const update_state = owned_chunked_update.?;
                delta_bytes = update_state.bytes;
                loss_val = update_state.loss;
                std.log.info("DiLoCo received chunked update from worker {}: {} bytes at 0x{x}", .{
                    response.sender_node,
                    delta_bytes.len,
                    @intFromPtr(delta_bytes.ptr),
                });
            } else {
                ...
                const reader = try binary_protocol.WorkerUpdatePayload.Reader.init(capnp_bytes);
                defer reader.deinit();
                delta_bytes = try reader.getUpdatedParams();
                loss_val = try reader.getLoss();
                std.log.info("DiLoCo received legacy update from worker {}: {} bytes at 0x{x}", .{
                    response.sender_node,
                    delta_bytes.len,
                    @intFromPtr(delta_bytes.ptr),
                });
            ...
            std.log.info("DiLoCo accumulating worker {} update into 0x{x} ({} floats)", .{
                response.sender_node,
                @intFromPtr(accumulator.ptr),
                num_floats,
            });
            ...
            switch (self.config.dtype) {
                .f32 => {
                    var offset: usize = 0;
                    for (0..num_floats) |i| {
                        const bits = std.mem.readInt(u32, delta_bytes[offset..][0..@sizeOf(f32)], .little);
                        const val: f32 = @bitCast(bits);
                        accumulator[i] += val;
                        offset += @sizeOf(f32);
                    }
            ...
            total_loss += loss_val;
            collected_count += 1;
            std.log.info("DiLoCo accumulated {}/{} worker updates for request {} round {}", .{
                collected_count,
                dispatch.workers_assigned,
                dispatch.request_id,
                dispatch.round_id,
        ...
        std.log.info("DiLoCo averaging {} accumulated gradients", .{num_floats});
        ...
        for (0..num_floats) |i| {
            ...
            if (std.math.isNan(accumulator[i]) or std.math.isInf(accumulator[i])) {
                accumulator[i] = 0.0;
                nan_grad_count += 1;
            }
        }
        ...
        std.log.info("DiLoCo applying averaged gradients across {} tensors", .{param_tensors.len});

        for (param_tensors, 0..) |*param_tensor_ptr, i| {
            if (i == 0 or (i + 1) % 25 == 0 or (i + 1) == param_tensors.len) {
                std.log.info("DiLoCo updating tensor {}/{}", .{ i + 1, param_tensors.len });
            }
            const shape = self.parameter_shapes[i];
            var elem_count: usize = 1;
            for (shape) |dim| elem_count *= @intCast(dim);
            ...
            switch (self.config.dtype) {
                .f32 => {
                    @memcpy(std.mem.sliceAsBytes(master_f32_buf), master_raw_bytes);
                },
                .bf16 => {
                    var off: usize = 0;
                    for (0..elem_count) |k| {
            ...
            const grad_slice = accumulator[acc_offset .. acc_offset + elem_count];
    ...
    /// Save recovery state to disk for worker_fabric resilience
    fn saveRecoveryState(self: *Self) !void {
        ...
        std.fs.cwd().makePath(recovery_dir) catch |err| {
            std.log.warn("Failed to create recovery directory '{s}': {}", .{ recovery_dir, err });
            return err;
        };
