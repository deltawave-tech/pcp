...
pub const StreamingDiLoCo = struct {
    ...
    pub fn init(allocator: Allocator, coordinator: *WorkerFabricController, config: StreamingDiLoCoConfig, mlir_builder: *ops.MLIRBuilder) !Self {
    ...
    fn parseFragmentRound(payload: std.json.ObjectMap) !usize {
        ...
        return switch (round_value) {
            .integer => |value| blk: {
                if (value < 0) return error.InvalidFragmentRound;
    ...
    pub fn run(ptr: *anyopaque) !void {
        ...
        while (self.status == .running) {
            ...
            // Note: We'll modify WorkerFabricController to expose this queue shortly.
    ...
    fn queuePendingMessage(
        ...
        for (frag_state.pending_messages.items) |pending| {
            ...
            const pending_round = parseFragmentRound(pending_payload) catch |err| {
                std.log.warn("Ignoring malformed pending fragment update from worker {}: {}", .{ pending.sender_node, err });
                continue;
            };
    ...
    fn processPendingMessages(self: *Self, frag_state: *FragmentState, expected_workers: usize) anyerror!void {
        while (frag_state.current_round < self.config.max_epochs) {
            ...
            while (i < frag_state.pending_messages.items.len) {
                ...
                const pending_round = parseFragmentRound(pending.data.object) catch |err| {
                    std.log.warn("Dropping malformed pending fragment update from worker {}: {}", .{ pending.sender_node, err });
    ...
    fn broadcastStartStreaming(self: *Self, workers: []const WorkerConnection) !void {
        ...
        for (workers) |worker| {
            ...
            try payload.put(data_assignment.field_name, assignment_value.value);
