...
pub const StreamingDiLoCo = struct {
    ...
    fn processFragmentMessage(self: *Self, msg: message.MessageEnvelope, expected_workers: usize) !void {
        ...
        try self.accumulateFragmentGradients(frag_state, payload);

        try frag_state.received_from.put(msg.sender_node, {});

        std.log.info("Fragment {} received from worker {} ({}/{})", .{
            fragment_id, msg.sender_node, frag_state.received_from.count(), expected_workers
        });

        // Check if we reached consensus for this fragment
        if (frag_state.received_from.count() >= expected_workers) {
            try self.applyFragmentUpdate(frag_state, expected_workers);
        }
    }

    /// Apply Host Optimizer and Broadcast updated Fragment
    fn applyFragmentUpdate(self: *Self, frag_state: *FragmentState, worker_count: usize) !void {
        std.log.info("Applying Nesterov update to Fragment {}...", .{frag_state.id});
        ...
        try self.broadcastFragment(frag_state.id);
    }
    ...
    fn broadcastFragment(self: *Self, fragment_id: usize) !void {
        std.log.info("Broadcasting updated Fragment {} to workers...", .{fragment_id});
