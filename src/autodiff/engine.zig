...
fn getVjpFn(op_name: []const u8) ?VJPFn {
    ...
    } else if (std.mem.eql(u8, op_name, "stablehlo.dynamic_update_slice")) {
        return vjp_rules.dynamicUpdateSliceVJP;
...
pub fn buildGradientGraph(
    ...
    return buildGradientGraphForInputIndices(allocator, builder, forward_fn, gradient_clip_min, gradient_clip_max, null);
}

/// Build a gradient graph that optionally returns gradients for only a selected
/// subset of forward-function input indices. When `return_input_indices` is
/// null, this preserves the legacy contract and returns one gradient per input.
pub fn buildGradientGraphForInputIndices(
    allocator: Allocator,
    builder: *MLIRBuilder,
    forward_fn: mlir.Operation,
    gradient_clip_min: f64,
    gradient_clip_max: f64,
    return_input_indices: ?[]const usize,
) !mlir.Operation {
    ...
    const gradient_fn = try createGradientFunction(builder, forward_fn, grad_fn_name, return_input_indices);
    ...
    try finalizeGradientFunction(allocator, builder, gradient_fn, forward_fn, &adjoint_map, &value_map, return_input_indices);
...
fn createGradientFunction(builder: *MLIRBuilder, forward_fn: mlir.Operation, name: []const u8, return_input_indices: ?[]const usize) !mlir.Operation {
    ...
    if (return_input_indices) |indices| {
        for (indices) |i| {
            if (i >= num_args) return error.TrainableParameterIndexOutOfBounds;
            const arg = forward_block.getArgument(i);
...
fn finalizeGradientFunction(
    ...
    std.debug.print("\nLooking for gradients for selected forward function arguments:\n", .{});
    if (return_input_indices) |indices| {
        for (indices) |i| {
            if (i >= num_args) return error.TrainableParameterIndexOutOfBounds;
            try appendInputGradient(allocator, builder, forward_block, i, adjoint_map, value_map, &input_gradients);
        }
    } else {
        for (0..num_args) |i| {
            try appendInputGradient(allocator, builder, forward_block, i, adjoint_map, value_map, &input_gradients);
...
fn appendInputGradient(
    allocator: Allocator,
    builder: *MLIRBuilder,
    forward_block: mlir.Block,
    i: usize,
    adjoint_map: *std.AutoHashMap(mlir.Value, mlir.Value),
    value_map: *std.AutoHashMap(mlir.Value, mlir.Value),
    input_gradients: *std.ArrayList(mlir.Value),
) !void {
    const arg = forward_block.getArgument(i);
    if (i < 5) {
    ...
    const mapped_arg = value_map.get(arg) orelse {
        ...
        const zero_tensor = try zeroGradientForArg(allocator, builder, arg);
        try input_gradients.append(zero_tensor);
...
fn zeroGradientForArg(allocator: Allocator, builder: *MLIRBuilder, arg: mlir.Value) !mlir.Value {
    ...
    return (try ops.constant(builder, 0.0, shape, elem_type)).value;
}
