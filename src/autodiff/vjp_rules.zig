...
pub fn powerVJP(
    ...
    // Use y/base instead of base^(exp-1) to avoid NaNs for negative bases when exp-1
    // is not a compile-time constant (some backends lower pow(x, y) via exp(y*log(x))).
    //
    // d(base^exp)/d(base) = grad_out * exp * (base^exp / base)  (for base != 0)
    // We define base^exp / base as 0 when base==0 (avoids 0/0 -> NaN; correct for exp=2).
...
pub fn concatenateVJP(
    ...
    const concat_dim = blk: {
        const name = c.stringRefFromString("dimension");
        const attr = c.operationGetAttributeByName(original_op.handle, name);
        if (@intFromPtr(attr.ptr) != 0) break :blk c.integerAttrGetValueInt(attr);
        break :blk try inferConcatDimension(builder, original_op, primals);
    };
    const rank = result_shape.len;
    var offset: i64 = 0;
