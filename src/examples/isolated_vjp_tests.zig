...
/// Test dynamic_update_slice VJP numerically.
/// f(base, update) = dynamic_update_slice(base, update, [0, 1]) ** 2
/// with incoming gradient all ones.
/// base=[1,2,3,4], update=[10,20] -> updated=[1,10,20,4]
/// Expected d_base=[2,0,0,8], d_update=[20,40].
pub fn testDynamicUpdateSliceVJP(allocator: Allocator) !void {
    std.debug.print("\n=== Testing dynamicUpdateSliceVJP Isolated Execution ===\n", .{});
    try initGlobalMLIRContext(allocator);
    const context = global_mlir_context.?.getContext();
    var helper = ExecutionHelper{ .allocator = allocator, .backend = .cpu };
    var builder = try MLIRBuilder.init(allocator, context);
    defer builder.deinit();

    const f32_type = mlir.Type.f32Type(context);
    const i64_type = mlir.Type.i64Type(context);
    const base_type = mlir.Type.rankedTensorType(context, &.{ 1, 4 }, f32_type);
    const update_type = mlir.Type.rankedTensorType(context, &.{ 1, 2 }, f32_type);
    const func_type = try mlir.Type.functionType(allocator, context, &.{ base_type, update_type }, &.{base_type});

    const fwd_result = try builder.createFunction("forward_dynamic_update_slice", func_type);
    builder.setInsertionBlock(fwd_result.entry_block);

    const base = fwd_result.entry_block.getArgument(0);
    const update = fwd_result.entry_block.getArgument(1);
    const zero = (try ops.constant(&builder, 0.0, &.{}, i64_type)).value;
    const one = (try ops.constant(&builder, 1.0, &.{}, i64_type)).value;

    const updated_op = try builder.createAndAttach(
        "stablehlo.dynamic_update_slice",
        &.{ base, update, zero, one },
        &.{base_type},
        .{},
    );
    const squared_op = try builder.createAndAttach(
        "stablehlo.multiply",
        &.{ updated_op.getResult(0), updated_op.getResult(0) },
        &.{base_type},
        .{},
    );
    _ = try builder.createAndAttach("func.return", &.{squared_op.getResult(0)}, &.{}, .{});

    _ = try autodiff.buildGradientGraph(allocator, &builder, fwd_result.func_op, -100.0, 100.0);

    const base_input = [_]f32{ 1.0, 2.0, 3.0, 4.0 };
    const update_input = [_]f32{ 10.0, 20.0 };
    const grad_out = [_]f32{ 1.0, 1.0, 1.0, 1.0 };
    var inputs_bytes = [_][]const u8{
        std.mem.sliceAsBytes(&base_input),
        std.mem.sliceAsBytes(&update_input),
        std.mem.sliceAsBytes(&grad_out),
    };
    var shapes = [_][]const i64{ &[_]i64{ 1, 4 }, &[_]i64{ 1, 2 }, &[_]i64{ 1, 4 } };

    const grad_outputs = try helper.executeModule(builder.module, "forward_dynamic_update_slice_grad", &inputs_bytes, &shapes, null);
    defer {
        for (grad_outputs) |o| allocator.free(o);
        allocator.free(grad_outputs);
    }

    try std.testing.expectEqual(@as(usize, 2), grad_outputs.len);
    const grad_base = std.mem.bytesAsSlice(f32, grad_outputs[0]);
    const grad_update = std.mem.bytesAsSlice(f32, grad_outputs[1]);

    const expected_base = [_]f32{ 2.0, 0.0, 0.0, 8.0 };
    const expected_update = [_]f32{ 20.0, 40.0 };
    for (expected_base, 0..) |expected, i| {
        try std.testing.expectApproxEqAbs(expected, grad_base[i], 1e-5);
    }
    for (expected_update, 0..) |expected, i| {
        try std.testing.expectApproxEqAbs(expected, grad_update[i], 1e-5);
    }

    std.debug.print("✓ dynamicUpdateSliceVJP gradients verified.\n", .{});
}
...
pub fn main() !void {
    ...
    if (args.len == 2 and std.mem.eql(u8, args[1], "dynamic-update-slice")) {
        try testDynamicUpdateSliceVJP(allocator);
        return;
    }
