...
pub const ModelMetadata = struct {
    ...
    parameter_dtypes: ?[]tensor.DType = null,
    data_input_shapes: [][]i64,
    data_input_dtypes: []tensor.DType,
    trainable_parameter_indices: ?[]usize = null,
    ...
    pub fn deinit(self: *ModelMetadata) void {
        ...
        if (self.parameter_dtypes) |dtypes| self.allocator.free(dtypes);
        ...
        if (self.trainable_parameter_indices) |indices| self.allocator.free(indices);
    }
    ...
    pub fn saveToFile(self: ModelMetadata, path: []const u8) !void {
        ...
        try writer.writeAll("  ]");
        if (self.parameter_dtypes) |param_dtypes| {
            try writer.writeAll(",\n  \"parameter_dtypes\": [\n");
            for (param_dtypes, 0..) |dt, i| {
                const s = dtypeName(dt);
                try writer.print("    \"{s}\"", .{s});
                if (i < param_dtypes.len - 1) try writer.writeAll(",");
                try writer.writeAll("\n");
            }
            try writer.writeAll("  ]");
        }
        ...
        if (self.trainable_parameter_indices) |indices| {
            ...
            for (indices, 0..) |index, i| {
                ...
                if (i < indices.len - 1) try writer.writeAll(",");
                try writer.writeAll("\n");
            }
            try writer.writeAll("  ]");
        }
    ...
    pub fn loadFromFile(allocator: std.mem.Allocator, path: []const u8) !ModelMetadata {
        ...
        const Parsed = struct {
            parameter_shapes: [][]i64,
            parameter_dtypes: ?[][]const u8 = null,
            data_input_shapes: [][]i64,
        ...
        const param_dtypes = if (parsed.value.parameter_dtypes) |dtypes_raw| blk: {
            if (dtypes_raw.len != param_shapes.len) return error.ParameterDTypeCountMismatch;
            var dtypes = try allocator.alloc(tensor.DType, dtypes_raw.len);
            errdefer allocator.free(dtypes);
            for (dtypes_raw, 0..) |s, i| {
                dtypes[i] = parseDtypeName(s);
            }
            break :blk dtypes;
        } else null;
        ...
        const trainable_indices = if (parsed.value.trainable_parameter_indices) |indices|
            try allocator.dupe(usize, indices)
        else
            null;
...
fn dtypeName(dt: tensor.DType) []const u8 {
    return switch (dt) {
        .f32 => "f32",
        .f64 => "f64",
        .f16 => "f16",
...
fn parseDtypeName(s: []const u8) tensor.DType {
