...
pub const Attention = struct {
    ...
    pub fn forward(self: *Attention, x: Tensor) !Tensor {
        ...
        var q_tensor = try Tensor.zeros(self.allocator, &[_]usize{ batch_size, self.n_embd }, qkv.dtype, qkv.backend);
        defer q_tensor.deinit();

        var k_tensor = try Tensor.zeros(self.allocator, &[_]usize{ batch_size, self.n_embd }, qkv.dtype, qkv.backend);
        defer k_tensor.deinit();

        var v_tensor = try Tensor.zeros(self.allocator, &[_]usize{ batch_size, self.n_embd }, qkv.dtype, qkv.backend);
...
pub const GPT2 = struct {
    ...
    lm_head: Tensor(f32), // Language modeling head
    ...
    pub fn forwardWithGradAndLoss(self: *GPT2, input_ids: Tensor, targets: Tensor) !struct { logits: Tensor, loss: Tensor, grads: std.AutoHashMap(*Tensor, Tensor) } {
