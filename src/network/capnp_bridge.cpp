...
// --- WorkerUpdatePayload ---
CapnpMessageBuilder* new_worker_update_payload_builder() {
    auto cpp_builder = new CapnpMessageBuilder();
    cpp_builder->builder.initRoot<WorkerUpdatePayload>();
    return cpp_builder;
}

void set_worker_update_payload_params(CapnpMessageBuilder* builder, const uint8_t* data, size_t size) {
    auto root = builder->builder.getRoot<WorkerUpdatePayload>();
    root.setUpdatedParams(kj::ArrayPtr<const kj::byte>(data, size));
}

void set_worker_update_payload_loss(CapnpMessageBuilder* builder, float loss) {
    auto root = builder->builder.getRoot<WorkerUpdatePayload>();
    root.setLoss(loss);
}
...
int get_worker_update_payload_params(CapnpMessageReader* reader, const uint8_t** data, size_t* size) {
    auto payload = reader->reader.getRoot<WorkerUpdatePayload>();
...
int get_worker_update_payload_loss(CapnpMessageReader* reader, float* loss) {
    auto payload = reader->reader.getRoot<WorkerUpdatePayload>();
