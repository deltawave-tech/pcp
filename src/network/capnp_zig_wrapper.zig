...
pub const WorkerUpdatePayload = struct {
    updated_params: []const u8,
    loss: f32,

    pub fn serialize(self: @This(), allocator: Allocator) ![]u8 {
        const builder = c.new_worker_update_payload_builder();
        defer c.free_builder(builder);
        c.set_worker_update_payload_params(builder, self.updated_params.ptr, self.updated_params.len);
        c.set_worker_update_payload_loss(builder, self.loss);
    ...
    pub const Reader = struct {
        ...
        pub fn getUpdatedParams(self: Reader) ![]const u8 {
            var data_ptr: [*c]const u8 = undefined;
            var size: usize = undefined;
            if (c.get_worker_update_payload_params(self.reader_ptr, &data_ptr, &size) == 0) {
                return error.DeserializationFailed;
            }
            return data_ptr[0..size];
        }
        ...
        pub fn getLoss(self: Reader) !f32 {
            ...
            if (c.get_worker_update_payload_loss(self.reader_ptr, &loss) == 0) {
                return error.DeserializationFailed;
            }
            return loss;
        }
