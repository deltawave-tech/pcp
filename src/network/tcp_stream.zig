...
pub const TcpStreamManager = struct {
    ...
    pub fn send(stream: net.Stream, envelope: MessageEnvelope, allocator: Allocator) !void {
        _ = allocator;
        // Use an independent allocator for transient send buffers so concurrent controller
        // work doesn't contend on the long-lived shared allocator.
        const json_buffer = try envelope.asJsonString(std.heap.c_allocator);
