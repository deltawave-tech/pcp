...
pub const FederationClient = struct {
    ...
    pub fn syncOnce(self: *Self) !void {
        ...
        var advertised_services = std.ArrayList(federation_types.ServiceAdvertisement).init(self.allocator);
        defer advertised_services.deinit();
        for (services) |service| {
            try advertised_services.append(.{
        ...
        const body = try std.json.stringifyAlloc(self.allocator, .{
            ...
            .registered_services = advertised_services.items.len,
            .services = advertised_services.items,
        ...
        var parsed = try std.json.parseFromSlice(federation_types.ConnectResponse, self.allocator, response_body.items, .{ .ignore_unknown_fields = true });
        defer parsed.deinit();
