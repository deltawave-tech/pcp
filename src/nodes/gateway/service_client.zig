...
pub const GatewayClient = struct {
    ...
    pub fn initFromEnv(allocator: Allocator, default_service_id: []const u8) !?Self {
        ...
        const executor_id = std.process.getEnvVarOwned(allocator, "PCP_GATEWAY_EXECUTOR_ID") catch |err| switch (err) {
            error.EnvironmentVariableNotFound => null,
            else => return err,
        };
        errdefer if (executor_id) |value| allocator.free(value);
    ...
    pub fn registerService(
        ...
        const body = try std.json.stringifyAlloc(self.allocator, .{
            ...
            .worker_count = workers_connected,
            .ready_worker_count = workers_available,
            .workers_connected = workers_connected,
            .workers_ready = workers_ready,
            .workers_available = workers_available,
            .workers_dispatchable = workers_available,
