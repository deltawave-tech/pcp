...
pub const Worker = struct {
    ...
    // Maps fragment_id -> Received MessageEnvelope
    rx_map: std.AutoHashMap(usize, message.MessageEnvelope),
    ...
    pub fn init(allocator: Allocator, backend: WorkerBackend, supervisor_id: ?i64) !Self {
        return Self{
            ...
            .tx_mutex = std.Thread.Mutex{},
            .tx_cond = std.Thread.Condition{},
            .rx_map = std.AutoHashMap(usize, message.MessageEnvelope).init(allocator),
            .rx_mutex = std.Thread.Mutex{},
            .rx_cond = std.Thread.Condition{},
        };
    }
    ...
    fn txLoop(self: *Self) void {
        ...
        while (self.is_running) {
            self.tx_mutex.lock();
            while (self.tx_queue.items.len == 0 and self.is_running) {
        ...
        while (self.is_running) {
            ...
            if (std.mem.eql(u8, msg.msg_type, MessageType.FRAGMENT_READY)) {
                ...
                if (msg.clone(self.allocator)) |msg_clone| {
                    self.rx_mutex.lock();
                    self.rx_map.put(frag_id, msg_clone) catch |err| {
                    ...
                    std.log.info("Worker {}: Network received Fragment {}", .{self.node_id.?, frag_id});
                } else |err| {
    ...
    fn handleStartStreamingLoop(self: *Self, msg: MessageEnvelope) !void {
        ...
        // Start the Async I/O threads
        std.log.info("Worker {}: Starting async I/O threads", .{self.node_id.?});
        const tx_thread = try std.Thread.spawn(.{}, txLoop, .{self});
        const rx_thread = try std.Thread.spawn(.{}, rxLoop, .{self});
        tx_thread.detach();
        rx_thread.detach();

        // Main continuous training loop
        const max_steps: usize = self.inner_steps * 10; // Run for multiple cycles
        std.log.info("Worker {}: Entering main streaming loop (max_steps={})", .{self.node_id.?, max_steps});
        while (self.current_step < max_steps and self.is_running) {
            ...
            for (0..self.num_fragments) |p| {
                // Use floating point for accurate fragment timing, then convert to integer
                const t_p = @as(usize, @intFromFloat(@floor(@as(f64, @floatFromInt(p)) * @as(f64, @floatFromInt(self.inner_steps)) / @as(f64, @floatFromInt(self.num_fragments)))));
                if (t > (t_p + self.overlap_tau)) {
                    if ((t - t_p - self.overlap_tau) % self.inner_steps == 0) {
                        ...
                        try self.receiveAndMergeFragment(p, &device_params);
                    }
                }
            }
            ...
            for (0..self.num_fragments) |p| {
                ...
                if (t > t_p and (t - t_p) % self.inner_steps == 0) {
                    ...
                    try self.extractAndSendFragment(p, device_params);
                }
            }
        }
    ...
    fn extractAndSendFragment(self: *Self, fragment_id: usize, device_params: []IreeBackend.DeviceBuffer) !void {
        std.log.info("Worker {}: [t={}] Extracting and sending Fragment {}", .{self.node_id.?, self.current_step, fragment_id});
        ...
        const msg = tcp_stream.createMessage(
            ...
            message.MessageType.FRAGMENT_UPDATE, 1,
        ...
        std.log.info("Worker {}: [t={}] Pushed Fragment {} to Tx Queue.", .{self.node_id.?, self.current_step, fragment_id});
    }

    /// Pops merged Fragment P from the Rx Map, Alpha blends it, and uploads to GPU
    fn receiveAndMergeFragment(self: *Self, fragment_id: usize, device_params: *[]IreeBackend.DeviceBuffer) !void {
        std.log.info("Worker {}: [t={}] Time to merge Fragment {}...", .{self.node_id.?, self.current_step, fragment_id});
        ...
        while (!self.rx_map.contains(fragment_id) and self.is_running) {
            std.log.warn("Worker {}:[t={}] Sync Stall! Fragment {} hasn't arrived yet. Blocking...", .{self.node_id.?, self.current_step, fragment_id});
            self.rx_cond.wait(&self.rx_mutex);
        }
        ...
        const kv = self.rx_map.fetchRemove(fragment_id).?;
        const msg = kv.value;
        self.rx_mutex.unlock();
        ...
        std.log.info("Worker {}:[t={}] Fragment {} merged successfully. Resuming compute.", .{self.node_id.?, self.current_step, fragment_id});
    }
