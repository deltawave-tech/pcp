...
/// Used for WorkerFabricController/Coordinator updates on the CPU.
pub const Nesterov = struct {
    ...
    pub fn deinit(self: *Self) void {
        if (self.initialized) {
            for (self.velocities) |v| {
                if (v.len > 0) self.allocator.free(v);
            }
    ...
    /// Allocate velocity buffers only for selected trainable parameter indices.
    /// The outer slice keeps parameter-index addressing stable, while the
    /// backing velocity storage remains proportional to the configured subset.
    pub fn initParametersForIndices(self: *Self, shapes: [][]const i64, trainable_indices: []const usize) !void {
        if (self.initialized) return;

        self.velocities = try self.allocator.alloc([]f32, shapes.len);
        errdefer self.allocator.free(self.velocities);
        @memset(self.velocities, &[_]f32{});

        for (trainable_indices) |index| {
            if (index >= shapes.len) return error.TrainableParameterIndexOutOfBounds;
            if (self.velocities[index].len != 0) return error.DuplicateTrainableParameterIndex;

            var elem_count: usize = 1;
            for (shapes[index]) |dim| elem_count *= @intCast(dim);

            self.velocities[index] = try self.allocator.alloc(f32, elem_count);
            @memset(self.velocities[index], 0.0);
        }
        self.initialized = true;
    }
    ...
    pub fn deserialize(self: *Nesterov, data: []const u8) !void {
        ...
        if (offset != data.len) return error.CorruptSaveFile;
    }
};

test "nesterov allocates velocity only for configured trainable subset" {
    const allocator = std.testing.allocator;
    var opt = Nesterov.init(allocator, .{});
    defer opt.deinit();
    ...
    const trainable = [_]usize{ 0, 2 };

    try opt.initParametersForIndices(@constCast(shapes[0..]), trainable[0..]);

    try std.testing.expect(opt.initialized);
    try std.testing.expectEqual(@as(usize, 3), opt.velocities.len);
    try std.testing.expectEqual(@as(usize, 4), opt.velocities[0].len);
    try std.testing.expectEqual(@as(usize, 0), opt.velocities[1].len);
    try std.testing.expectEqual(@as(usize, 20), opt.velocities[2].len);
}
