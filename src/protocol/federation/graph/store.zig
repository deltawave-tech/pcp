...
const federation_types = @import("../types.zig");
...
pub const GraphStore = struct {
    ...
    pub fn applyMutationsJson(self: *Self, allocator: Allocator, request: types.MutateRequest) ![]u8 {
        ...
        for (request.mutations) |mutation| {
            ...
            const payload_json = try json_util.stringifyOrDefault(allocator, mutation.payload, "null");
            defer allocator.free(payload_json);
            const provenance_json = try json_util.stringifyOrDefault(allocator, mutation.provenance, "null");
