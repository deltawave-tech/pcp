...
pub const Neo4jGraphStore = struct {
    ...
    pub fn counts(self: *Self) types.GraphCounts {
        const statement =
            ...
            \\}
        ;
        const response = self.runStatement(self.allocator, statement, null) catch |err| {
            std.log.warn("Failed to query Neo4j graph counts: {}", .{err});
            return .{};
        };
        defer response.deinit();
        return parseCounts(response.value) catch |err| {
            std.log.warn("Failed to parse Neo4j graph counts: {}", .{err});
            return .{};
        };
    }
    ...
    pub fn applyMutation(self: *Self, gateway_id: []const u8, lab_id: []const u8, mutation: types.MutationRequest, mutation_type: types.MutationType, visibility: types.Visibility, provenance_json: []const u8, now: i64) !void {
        ...
        const payload_json = try json_util.stringifyOrDefault(self.allocator, mutation.payload, "null");
