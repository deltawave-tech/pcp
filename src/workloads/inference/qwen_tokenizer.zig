...
const RenderChatResponse = struct {
    ok: bool,
    text: ?[]const u8 = null,
...
const EosTokenResponse = struct {
    ok: bool,
    eos_token_id: ?i64 = null,
    @"error": ?[]const u8 = null,
};
...
pub const QwenTokenizer = struct {
    ...
    pub fn renderChat(self: *Self, chat_template: []const u8, messages: std.json.Array) ![]u8 {
        self.mutex.lock();
        defer self.mutex.unlock();
        ...
        var parsed = try std.json.parseFromSlice(RenderChatResponse, self.allocator, response_line, .{ .ignore_unknown_fields = true });
        defer parsed.deinit();
    ...
    pub fn eosTokenId(self: *Self) !?i64 {
        self.mutex.lock();
        defer self.mutex.unlock();
        ...
        var parsed = try std.json.parseFromSlice(EosTokenResponse, self.allocator, response_line, .{ .ignore_unknown_fields = true });
        defer parsed.deinit();

        if (!parsed.value.ok) {
            return error.TokenizerFailed;
        }
        return parsed.value.eos_token_id;
    }
