...
pub const Tokenizer = struct {
    ...
    pub fn renderChat(self: *Self, chat_template_name: []const u8, messages: std.json.Array, vision_enabled: bool) ![]u8 {
        try chat_template.validateMessages(messages, vision_enabled);
        return switch (self.kind) {
            .qwen => try self.qwen_tokenizer.?.renderChat(chat_template_name, messages),
            .byte, .char, .u16 => try chat_template.render(self.allocator, chat_template_name, messages, vision_enabled),
        };
    }

    pub fn eosTokenId(self: *Self) !?i64 {
        return switch (self.kind) {
            .qwen => try self.qwen_tokenizer.?.eosTokenId(),
            .byte, .char, .u16 => null,
        };
    }
};
