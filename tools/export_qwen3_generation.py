...
try:
    ...
    with open(OUTPUT_PATH, "w") as f:
        f.write(str(prog))
    meta = {
        "parameter_shapes": [list(p.shape) for p in param_values],
        "data_input_shapes": [
            list(dummy_input_ids.shape),
            list(dummy_pos_ids.shape),
            *[list(t.shape) for t in dummy_kv_flat],
        ],
        "data_input_dtypes": ["i64", "i64", *["f32" for _ in dummy_kv_flat]],
    }
