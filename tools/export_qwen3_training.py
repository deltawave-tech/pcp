...
class QwenGRPOLogic(nn.Module):
    ...
    def token_nll(self, logits, labels):
        ...
        # Keep the label path i32. torch.gather requires int64 indices in eager
        # PyTorch, while embedding lowers to stablehlo.gather with i32 indices.
        flat_log_probs = log_probs.reshape(-1, 1)
        flat_labels = labels.reshape(-1)
        token_positions = torch.arange(flat_labels.shape[0], device=labels.device, dtype=TOKEN_DTYPE)
        linear_indices = token_positions * logits_f32.shape[-1] + flat_labels
        selected_log_probs = torch.nn.functional.embedding(linear_indices, flat_log_probs).reshape(labels.shape)
        return -selected_log_probs

    def forward(self, input_ids, attention_mask, advantages):
        causal_mask = get_causal_mask(input_ids.device)
        position_ids = torch.arange(SEQ_LEN, device=input_ids.device, dtype=TOKEN_DTYPE).unsqueeze(0).expand(GROUP_SIZE, SEQ_LEN)
        ...
        token_losses = self.token_nll(shift_logits, shift_labels)
...
dummy_input_ids = torch.zeros((GROUP_SIZE, SEQ_LEN), dtype=TOKEN_DTYPE)
