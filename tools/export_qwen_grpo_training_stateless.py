...
# --- RoPE Constants (Stateless) ---
HEAD_DIM = config.hidden_size // config.num_attention_heads
ROPE_BASE = float(config.rope_theta)
INV_FREQ_LIST = [1.0 / (ROPE_BASE ** (2.0 * i / HEAD_DIM)) for i in range(HEAD_DIM // 2)]
print(f"RoPE: base={ROPE_BASE}, head_dim={HEAD_DIM}, inv_freq_len={len(INV_FREQ_LIST)}")


def stateless_rope(position_ids):
    """Compute RoPE cos/sin from position_ids using constant inv_freq."""
    inv_freq = torch.tensor(INV_FREQ_LIST, device=position_ids.device, dtype=torch.float32)
    inv_freq_expanded = inv_freq.view(1, 1, -1)  # [1, 1, head_dim/2]
    position_ids_expanded = position_ids.float().unsqueeze(-1)  # [batch, seq, 1]
    freqs = position_ids_expanded * inv_freq_expanded  # [batch, seq, head_dim/2]
    emb = torch.cat((freqs, freqs), dim=-1)  # [batch, seq, head_dim]
    cos = emb.cos().unsqueeze(1).to(dtype=DTYPE)  # [batch, 1, seq, head_dim]
...
def clean_qwen2_forward(self, input_ids, attention_mask, position_ids, past_key_values, inputs_embeds=None, use_cache=False, **kwargs):
    ...
    # Use stateless RoPE instead of self.rotary_emb
    position_embeddings = stateless_rope(position_ids)
...
class QwenGRPOWrapper(nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        # No buffers - causal mask computed inline
    ...
    def forward(self, input_ids, attention_mask, advantages):
        ...
        # 2. Causal Mask - computed inline (becomes constant in MLIR)
        causal_mask = torch.triu(torch.full((SEQ_LEN, SEQ_LEN), -1e4, dtype=DTYPE, device=input_ids.device), diagonal=1)
...
# Delete inv_freq buffer to prevent it being captured as input
if hasattr(model.model, 'rotary_emb') and hasattr(model.model.rotary_emb, 'inv_freq'):
    delattr(model.model.rotary_emb, 'inv_freq')
    print("Deleted model.model.rotary_emb.inv_freq buffer (using stateless RoPE)")

# --- EXPORT STRATEGY: Pre-transformed Functional Gradients ---

# 1. Extract State - Parameters only (no buffers needed with stateless approach)
params = dict(wrapper.named_parameters())
param_keys = list(params.keys())
param_values = [p.detach().requires_grad_(True) for p in params.values()]

print(f"Model has {len(param_keys)} parameters (no buffers - stateless)")

# 2. Define the Pure Functional Loss
# This function reconstructs the model state from arguments and runs forward
def functional_loss(params_tuple, input_ids, mask, adv):
    # Reconstruct the state dictionary (params only, no buffers)
    state_dict = {k: v for k, v in zip(param_keys, params_tuple)}
...
# Create wrapper function that takes flat inputs (params + data, no buffers)
def grad_fn_flat(*args):
    num_params = len(param_keys)
    curr_params = args[:num_params]
    input_ids = args[num_params]
    mask = args[num_params + 1]
    adv = args[num_params + 2]
    return compute_grads(curr_params, input_ids, mask, adv)

# Inputs: params + data (no buffers)
full_inputs = tuple(param_values + [dummy_input_ids, dummy_mask, dummy_advantages])
...
try:
    ...
    print(f"  Input count: {len(full_inputs)} (params={len(param_values)}, data=3)")
    ...
    with open(OUTPUT_PATH, "w") as f:
        f.write(str(prog))
    print("✓ Export Successful! MLIR has 290 params + 3 data inputs (no buffers), returns 290 gradients.")
